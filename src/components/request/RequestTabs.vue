...
const showAutoGeneratedHeaders = ref(false);
const isBulkEditMode = ref(false);
const bulkEditText = ref('');
const headersListRef = ref<HTMLElement | null>(null);
const autoHeadersSectionRef = ref<HTMLElement | null>(null);
const highlightedHeaderIndex = ref<number | null>(null);

// Track disabled auto-generated headers by their key
const disabledAutoHeaders = ref<Set<string>>(new Set());

function toggleAutoHeader(headerKey: string) {
  if (disabledAutoHeaders.value.has(headerKey)) {
    disabledAutoHeaders.value.delete(headerKey);
  } else {
    disabledAutoHeaders.value.add(headerKey);
  }
  // Force reactivity update
  disabledAutoHeaders.value = new Set(disabledAutoHeaders.value);
}

function isAutoHeaderDisabled(headerKey: string): boolean {
  return disabledAutoHeaders.value.has(headerKey);
}

interface AutoGeneratedHeader {
  key: string;
  value: string;
  enabled: boolean;
  source: 'system' | 'body' | 'auth';
}

// Auto-generated headers calculated dynamically from request state
const autoGeneratedHeaders = computed<AutoGeneratedHeader[]>(() => {
  const headers: AutoGeneratedHeader[] = [];
  const request = requestStore.currentRequest;

  // System headers (always present)
  headers.push(
    {
      key: 'User-Agent',
      value: 'Rune-API-Client/1.0',
      enabled: !isAutoHeaderDisabled('User-Agent'),
      source: 'system',
    },
    { key: 'Accept', value: '*/*', enabled: !isAutoHeaderDisabled('Accept'), source: 'system' },
    {
      key: 'Accept-Encoding',
      value: 'gzip, deflate, br',
      enabled: !isAutoHeaderDisabled('Accept-Encoding'),
      source: 'system',
    },
    {
      key: 'Accept-Language',
      value: 'en-US,en;q=0.9',
      enabled: !isAutoHeaderDisabled('Accept-Language'),
      source: 'system',
    },
    {
      key: 'Connection',
      value: 'keep-alive',
      enabled: !isAutoHeaderDisabled('Connection'),
      source: 'system',
    }
  );
  ...
  if (request.bodyType !== 'none') {
    ...
    switch (request.bodyType) {
      case 'raw':
        contentType = request.rawContentType || 'text/plain';
        break;
      case 'formData':
    ...
    if (contentType) {
      headers.push({
        key: 'Content-Type',
        value: contentType,
        enabled: !isAutoHeaderDisabled('Content-Type'),
        source: 'body',
      });
    }
  }
  ...
  if (request.body) {
    ...
    const length = encoder.encode(request.body).length;
    headers.push({
      key: 'Content-Length',
      value: String(length),
      enabled: !isAutoHeaderDisabled('Content-Length'),
      source: 'body',
    });
  ...
  // Authorization from auth config
  if (authType.value === 'bearer' && authConfig.value.token.trim()) {
    headers.push({
      key: 'Authorization',
      value: `Bearer ${authConfig.value.token.trim()}`,
      enabled: !isAutoHeaderDisabled('Authorization'),
      source: 'auth',
    });
  } else if (
...
const manualHeaderCount = computed(() => requestStore.currentRequest.headers.length);

async function addHeader() {
  const newIndex = requestStore.currentRequest.headers.length;
  requestStore.addHeader({ key: '', value: '' });

  highlightedHeaderIndex.value = newIndex;
  setTimeout(() => {
    if (highlightedHeaderIndex.value === newIndex) {
      highlightedHeaderIndex.value = null;
    }
  }, 1200);

  await nextTick();
  const rows = headersListRef.value?.querySelectorAll('[data-testid="manual-header-row"]');
  const lastRow = rows?.[rows.length - 1] as HTMLElement | undefined;
  lastRow?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  const firstTextInput = lastRow?.querySelector('input[type="text"]') as HTMLInputElement | null;
...
function isHeaderOverridden(autoHeaderKey: string): boolean {
  const manualHeaders = requestStore.currentRequest.headers;
  return manualHeaders.some(h => h.enabled && h.key.toLowerCase() === autoHeaderKey.toLowerCase());
}

function isManualHeaderOverriding(headerKey: string): boolean {
  const autoHeadersKeys = autoGeneratedHeaders.value.map(h => h.key.toLowerCase());
  return autoHeadersKeys.includes(headerKey.toLowerCase());
}

function restoreAutoHeader(autoHeaderKey: string) {
  // Remove all manual headers with this key to restore the auto one
  const newHeaders = requestStore.currentRequest.headers.filter(
    h => h.key.toLowerCase() !== autoHeaderKey.toLowerCase()
  );
  requestStore.setHeaders(newHeaders);
}
...
function toggleBulkEdit() {
  if (isBulkEditMode.value) {
    // Switching from bulk to table - apply changes
    applyBulkEdit();
  } else {
    // Switching from table to bulk - generate text
    generateBulkText();
  }
...
function generateBulkText() {
  const headers = requestStore.currentRequest.headers;
  const lines = headers.map(h => {
    ...
    line += `${h.key}: ${h.value}`;
    // Include description as inline comment if present
    if (h.description) {
      line += ` # ${h.description}`;
    }
    return line;
  });
  bulkEditText.value = lines.join('\n');
}

function applyBulkEdit() {
  const text = bulkEditText.value;
  const headers = parseBulkHeaders(text);
  requestStore.setHeaders(headers);
}

function parseBulkHeaders(text: string): HttpHeader[] {
  const headers: HttpHeader[] = [];

  // First, try to extract all curl -H headers from the entire text
  const curlHRegex = /-H\s*["']([^"']+)["']/g;
  const processedText = text.replace(curlHRegex, (_match, headerContent) => {
    // Process each curl -H header immediately
    const colonMatch = headerContent.match(/^([^:]+):\s*(.*)$/);
    if (colonMatch) {
    ...
    return ''; // Remove from text to avoid double processing
  });

  // Process remaining lines (non-curl format)
  const lines = processedText.split('\n');

  for (const line of lines) {
    const trimmedLine = line.trim();
    if (!trimmedLine) continue; // Skip empty lines
    if (trimmedLine.startsWith('curl')) continue; // Skip curl command
    ...
    // Extract inline description comment (everything after #)
    let description = '';
    const descMatch = workingLine.match(/#\s*(.+)$/);
    if (descMatch) {
      description = descMatch[1].trim();
      workingLine = workingLine.substring(0, workingLine.indexOf('#')).trim();
    }

    // Parse Key: Value format
    const colonMatch = workingLine.match(/^([^:]+):\s*(.*)$/);
    if (colonMatch) {
      const key = colonMatch[1].trim();
      const value = colonMatch[2].trim();
      if (key) {
        headers.push({ key, value, enabled, description });
        continue;
      }
    }

    // Parse "Key Value" format (no colon, first word is key)
    const spaceMatch = workingLine.match(/^([^\s]+)\s+(.+)$/);
    if (spaceMatch) {
...
async function copyBulkToClipboard() {
  try {
    await navigator.clipboard.writeText(bulkEditText.value);
  } catch (err) {
...
watch(showAutoGeneratedHeaders, async visible => {
  if (!visible) return;
  await nextTick();
  autoHeadersSectionRef.value?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
});
  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex-1 p-4 overflow-y-auto">
      ...
      <div v-if="activeTab === 'headers'" class="space-y-2">
        <!-- Header Presets Bar -->
        <div class="mb-3 rounded-lg border border-[#e5e7eb] bg-[#fafafa] p-1.5">
          <div class="flex items-center gap-2 overflow-x-auto pb-1">
            <div class="relative shrink-0">
              <button
                class="flex h-9 items-center gap-1.5 whitespace-nowrap rounded-md border border-[#d1d5db] bg-white px-2.5 text-sm font-medium text-[#374151] transition-colors hover:bg-[#f9fafb]"
              ...
              >
                ...
                <div
                  v-if="headerPresetStore.customPresets.length > 0"
                  class="border-t border-[#e5e7eb] py-1"
                >
                  <div class="px-3 py-1 text-xs font-medium text-[#6b7280] uppercase">
              ...
              >
                <p class="text-sm text-[#6b7280]">
                  {{ t('requestTabs.noPresets') || 'No presets yet' }}
                </p>
              </div>
            </div>

            <button
              class="flex h-9 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-md border border-[#99f6e4] bg-[#f0fdfa] px-2.5 text-sm font-medium text-[#0f766e] transition-colors hover:bg-[#ccfbf1]"
            ...
            >
              ...
              {{
                isBulkEditMode
                  ? t('requestTabs.tableView') || 'Table View'
                  : t('requestTabs.bulkEdit') || 'Bulk Edit'
              }}
            </button>
          </div>
        </div>
        ...
        <!-- Header Rows -->
        <div v-if="!isBulkEditMode" ref="headersListRef">
          <div
          ...
          >
            <div
              data-testid="manual-header-row"
              :class="[
                'flex items-center gap-2 rounded-md px-1.5 py-1 transition-colors',
                highlightedHeaderIndex === index ? 'bg-[#eff6ff]' : '',
              ]"
              :title="header.description || undefined"
            >
              <button
                :class="[
                  'w-5 h-5 rounded border flex items-center justify-center transition-colors',
                  header.enabled ? 'bg-[#3b82f6] border-[#3b82f6]' : 'bg-white border-[#d1d5db]',
                ]"
              ...
              <div class="flex-1 flex items-center gap-1">
                ...
                <!-- Override badge -->
                <span
                  v-if="isManualHeaderOverriding(header.key)"
                  data-testid="override-badge"
                  class="text-xs bg-[#f59e0b] text-white px-1.5 py-0.5 rounded"
                  title="This header overrides an auto-generated one"
                >
                  override
                </span>
                <!-- Info icon when description exists -->
                <span
              ...
              <button
                v-if="isManualHeaderOverriding(header.key)"
                data-testid="restore-auto-btn"
                class="p-1.5 text-[#3b82f6] hover:bg-[#eff6ff] rounded-md transition-colors"
                title="Restore auto-generated header"
                @click="restoreAutoHeader(header.key)"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"
              ...
              <button
                ...
                :class="{ 'text-[#3b82f6]': header.showDescription }"
                @click="header.showDescription = !header.showDescription"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"
            ...
            <div
              v-if="header.showDescription"
              data-testid="description-input-container"
            ...
            >
              <input
                ...
                class="flex-1 px-3 py-1.5 border border-[#e5e7eb] rounded-md text-sm text-[#6b7280] focus:outline-none focus:ring-2 focus:ring-[#3b82f6]"
              />
            </div>
          </div>
        </div>
        ...
        <!-- Bulk Edit Mode -->
        <div v-if="isBulkEditMode" class="space-y-2">
          <textarea
          ...
          <div class="flex gap-2">
            <button
              ...
              class="flex-1 flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium bg-[#3b82f6] text-white rounded-md hover:bg-[#2563eb] transition-colors"
              @click="applyBulkEdit"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
            ...
            <button
              ...
              class="flex items-center gap-2 px-4 py-2 text-sm font-medium bg-[#f3f4f6] text-[#374151] rounded-md hover:bg-[#e5e7eb] transition-colors"
              @click="copyBulkToClipboard"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path
                  stroke-linecap="round"
          ...
          <div class="text-xs text-[#9ca3af]">
            {{
              t('requestTabs.bulkEditHint') ||
              'Format: Key: Value (one per line). Lines starting with # are disabled. Supports curl -H "Key: Value" syntax.'
            }}
          </div>
        </div>
        <div class="flex items-center justify-between py-2 border-t border-[#e5e7eb] mt-2">
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-[#6b7280]">{{
              t('requestTabs.autoGenerated') || 'Auto-generated Headers'
            }}</span>
          </div>
          <button
            data-testid="auto-headers-toggle"
            class="flex items-center gap-1 text-xs text-[#3b82f6] hover:text-[#2563eb] transition-colors"
            @click="showAutoGeneratedHeaders = !showAutoGeneratedHeaders"
          >
            <svg
              :class="[
                'w-4 h-4 transition-transform',
                showAutoGeneratedHeaders ? 'rotate-180' : '',
              ]"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            ...
            {{ showAutoGeneratedHeaders ? t('common.hide') || 'Hide' : t('common.show') || 'Show' }}
          </button>
        </div>

        <!-- Auto-generated Headers Section -->
        <div
          v-if="showAutoGeneratedHeaders"
          ref="autoHeadersSectionRef"
          data-testid="auto-headers-section"
          class="space-y-2 border-t border-[#e5e7eb] pt-2"
        >
          <div class="text-xs text-[#9ca3af] mb-2">
            {{
              t('requestTabs.autoGeneratedHint') ||
              'These headers are automatically added by the system'
            }}
          </div>
          <div
            v-for="(header, index) in autoGeneratedHeaders"
            :key="'auto-' + index"
            data-testid="auto-header-row"
          ...
          >
            <button
              ...
              data-testid="auto-header-checkbox"
              @click="toggleAutoHeader(header.key)"
            >
              <svg
                v-if="header.enabled"
                class="w-3 h-3 text-white"
            ...
            <input
              ...
              type="text"
              readonly
              data-testid="auto-header-input"
              :class="[
                'flex-1 px-3 py-1.5 border rounded-md text-sm cursor-not-allowed',
                isHeaderOverridden(header.key)
                  ? 'bg-[#fef3c7] border-[#f59e0b] text-[#92400e] line-through'
            ...
            <div class="flex-1 flex flex-col">
              <input
                ...
                data-testid="auto-header-input"
                :class="[
                  'w-full px-3 py-1.5 border rounded-md text-sm cursor-not-allowed',
                  isHeaderOverridden(header.key)
                    ? 'bg-[#fef3c7] border-[#f59e0b] text-[#92400e] line-through'
                    : 'bg-[#f9fafb] border-[#e5e7eb] text-[#6b7280]',
                ]"
              />
              <span class="text-xs text-[#9ca3af] mt-0.5">{{
                header.source === 'body'
                  ? 'from Body'
                  : header.source === 'auth'
                    ? 'from Auth'
            ...
            <div class="w-8 flex items-center justify-center">
              <span
                v-if="isHeaderOverridden(header.key)"
                class="text-xs text-[#f59e0b] font-medium"
              >
                overridden
              </span>
            </div>
        ...
        >
          ...
          <span class="rounded-full bg-[#dbeafe] px-2 py-0.5 text-xs font-medium text-[#1d4ed8]">
            {{ manualHeaderCount }}
          </span>
        </button>
      </div>
      ...
      <div v-else-if="activeTab === 'params'" class="h-full flex flex-col">
        <div
          class="flex items-center justify-between px-4 py-2 border-b border-[#e5e7eb] bg-[#fafafa]"
        >
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-2">
          <div v-if="queryParams.length === 0" class="text-center py-8 text-[#9ca3af]">
            ...
            <p class="text-sm">
              {{ t('requestTabs.queryParamsHint') }}
            </p>
            <p class="text-xs mt-1">
              {{ t('requestTabs.queryParamsManual') }}
            </p>
          </div>
          <div v-for="(param, index) in queryParams" :key="index" class="flex items-center gap-2">
            <button
              :class="[
                'w-5 h-5 rounded border flex items-center justify-center transition-colors',
                param.enabled ? 'bg-[#3b82f6] border-[#3b82f6]' : 'bg-white border-[#d1d5db]',
              ]"
    ...
    <HeaderPresetManager v-model="showPresetManager" @apply="applyPreset" />
  </div>
</template>
